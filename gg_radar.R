# function for plotting ggradar, based on https://github.com/ricardo-bion/ggradar 
...
ggradar <- function(plot.data, 
  ...
  base <- base + geom_path(data = group$path, aes(x = x, y = y, 
                                                  group = group, 
                                                  colour = group),
                           alpha = line_alpha, size = group.line.width)
  ...
  if(TRUE){
    ...
    if(nrow(plot.data) > 1){
      base <- base + guides(colour=guide_legend(ncol=2))
    }
  }
  return(base)
}
