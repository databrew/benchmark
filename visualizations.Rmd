...
df <- left_join(df, competency_dict, by = 'combined_name') %>%
  dplyr::select(tab_name, competency, key, value)
df$key <- substr(df$key, nchar(df$key) - 7, nchar(df$key) - 7)
names(df) <- c('Area', 'Competency', 'Key', 'Score')
...
df <- df %>% dplyr::arrange(Area, Competency, Key)
